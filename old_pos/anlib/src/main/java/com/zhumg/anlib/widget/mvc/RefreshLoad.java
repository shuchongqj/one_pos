...
public class RefreshLoad {
    ...
    public void complete(boolean canLoadMore, boolean isEmpty, String info) {
        if (loadType == 1) {
            this.loadingView.hibe();
        ...
        if (this.loadmoreView != null) {
            if (canLoadMore) {
                ...
                //不为空的情况下
                this.loadmoreView.showComplete(info);
    ...
    public void complete(boolean canLoadMore, boolean isEmpty) {
        complete(canLoadMore, isEmpty, "没有更多数据了");
    }
    public void complete(boolean canLoadMore) {
        if (loadType == 1) {
            this.loadingView.hibe();
            this.listener.onLoading(true);
        } else if (loadType == 2) {
